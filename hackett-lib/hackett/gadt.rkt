...
;; this file started as a copy of hackett/private/adt.rkt
...
(begin-for-syntax
  ...
  (define-splicing-syntax-class data-constructor-spec
    #:attributes [tag [arg 1] len nullary? fixity out]
    #:commit
    #:description #f
    [pattern {~seq tag:id {~optional :fixity-annotation}}
             #:attr [arg 1] '()
             ...
             #:attr nullary? #t
             #:attr out #f]
    [pattern {~seq (~parens tag:id arg ...+ (~datum ->) out) {~optional :fixity-annotation}}
             #:attr len (length (attribute arg))
             #:attr nullary? #f]
         ...
         (define-values (match-pats- bodies-)
           (for/lists [match-pats- bodies-]
             ...
             (when DEBUG-CLAUSES
               (displayln '---------------clause)
               (print-current-subst))
             ;; save the context so it can be restored for each clause
             (define old-subst (current-type-context))
             ...
             (when DEBUG-CLAUSES
               (printf "body expected ty (inst'ed): ~a\n"
                       (syntax->datum
                        (apply-subst (current-type-context) t_body
                                     #:subst-rigid? #t))))
...
;;          (define patss-transposed (apply map list (attribute clause.pat)))
   ...
   #:do [; The resulting type of the case expression is the type we unified each clause against.
         ; TODO (GADTs): should be ok to get rid of this and just return t_body
         ; each body was already checked against (a properly instantiated) t_body
         ; so we know they typecheck
         ; - this happens to work because we reset the subst and are using an empty ctxt
         (define t_result
